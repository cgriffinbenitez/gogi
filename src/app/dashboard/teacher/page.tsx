...
function statusCfg(currentStatus: string | null, teacherFlag?: boolean | null): StatusCfg {
  if (teacherFlag) return { label: 'Needs Attention', bg: C.redLight,   border: C.red,    text: '#791F1F' };
  switch (currentStatus) {
    case 'mastered':      return { label: 'Mastered',        bg: C.greenLight, border: C.green,  text: '#27500A' };
    case 'intervening':
    case 'practicing':    return { label: 'In Intervention', bg: C.amberLight, border: C.amber,  text: '#633806' };
    case 'diagnostic':    return { label: 'In Diagnostic',   bg: C.blueLight,  border: C.blue,   text: '#0C447C' };
    default:              return { label: 'Not Started',     bg: C.light,      border: C.border, text: '#5F5E5A' };
  }
}
...
function StatCard({ value, label, color, sub }: { value: number; label: string; color: string; sub: string }) {
  return (
    <div style={{ background: C.white, border: `0.5px solid ${C.border}`, borderRadius: 8, padding: '12px 14px' }}>
...
function StatusBadge({ currentStatus, teacherFlag }: { currentStatus: string | null; teacherFlag?: boolean | null }) {
  const cfg = statusCfg(currentStatus, teacherFlag);
  return (
    <span style={{
      background: cfg.bg, border: `1px solid ${cfg.border}`,
      color: cfg.text, borderRadius: 5,
...
function ProgressBar({ pct }: { pct: number }) {
  return (
    <div style={{ flex: 1, background: C.light, height: 5, borderRadius: 3, overflow: 'hidden' }}>
      <div style={{ height: 5, width: `${Math.min(100, Math.max(0, pct))}%`, background: C.blue, borderRadius: 3 }} />
    </div>
  );
}
...
function TeacherNav({
  teacherName, school, activeTab, onTabChange,
}: {
  ...
  return (
    <nav style={{
      background: C.navy, height: 52, width: '100%', flexShrink: 0,
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              key={tab.key}
              onClick={() => {
                if (tab.key === 'questions') { router.push('/admin/questions'); return; }
                onTabChange(tab.key);
              }}
              style={{
...
function StudentDetailPanel({
  ...
  const cardStyle: React.CSSProperties = {
    background: C.white, border: `0.5px solid ${C.border}`, borderRadius: 8, padding: '12px 14px',
  };
  ...
  return (
    <div style={{
      background: 'rgba(31,78,121,0.06)', border: `1px solid ${C.blueLight}`,
      borderRadius: 10, padding: 16, marginTop: 0,
    }}>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
        <div>
          <div style={{ fontSize: 16, fontWeight: 700, color: C.navy }}>{student.full_name}</div>
          <div style={{ fontSize: 11, color: C.gray, marginTop: 2 }}>9th Grade · Period 3 · Pilot Cohort</div>
        </div>
        <button
          onClick={onClose}
          style={{ background: 'none', border: 'none', fontSize: 11, color: C.blue, cursor: 'pointer', fontFamily: FONTS.ui }}
        >
          ✕ Close
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        <div style={cardStyle}>
          ...
          {standards.map((std, i) => {
            ...
            const diagSession = sessions.find((s) => s.standard_id === std.id && s.phase === 'diagnostic');
            ...
            return (
              <div key={std.id} style={{
                ...
                <div>
                  ...
                  {cls && (
                    <div style={{ fontSize: 10, color: C.amber, marginTop: 2 }}>{clsLabel(cls)}</div>
                  )}
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            {([
              ...
              <div key={label} style={{
                background: C.light, borderRadius: 6,
                padding: '10px 8px', textAlign: 'center' as const,
              }}>
                <div style={{ fontSize: 18, fontWeight: 700, color: value != null ? color : C.border }}>
                  {value ?? '—'}
                </div>
        ...
        <div style={cardStyle}>
          ...
          {schemas.length === 0 ? (
            ...
            <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 7, fontSize: 10 }}>
              ...
              <span style={{
                ...
                color: C.amber, borderRadius: 4, padding: '1px 6px',
        ...
        <div style={cardStyle}>
          ...
          {sessions.length === 0 ? (
            ...
            return (
              <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'baseline', marginBottom: 7, fontSize: 10 }}>
                <span style={{ color: C.gray, minWidth: 52, flexShrink: 0 }}>{fmtDate(s.started_at)}</span>
                <span style={{ color: C.dark, flex: 1 }}>{label}</span>
                <span style={{ color: C.gray, flexShrink: 0 }}>{fmtDuration(s.time_spent_seconds)}</span>
              </div>
            );
          })}
        </div>
        ...
        {flagged.length > 0 && (
          <div style={{
            ...
            {flagged.map((p, i) => (
              <div key={i} style={{ fontSize: 10, color: '#791F1F', lineHeight: 1.5, marginBottom: 2 }}>
            ...
            <textarea
              ...
              style={{
                ...
                padding: 8, fontSize: 12, fontFamily: FONTS.ui,
                resize: 'vertical' as const, marginTop: 8,
                boxSizing: 'border-box' as const, minHeight: 60,
                outline: 'none',
              }}
            />
          </div>
...
export default function TeacherDashboardPage() {
  ...
  useEffect(() => {
    ...
    if (role === 'student')   { router.push('/dashboard/student'); return; }
    ...
    async function load() {
      try {
        ...
        const [
          ...
          supabase.from('standards').select('id, code, title').in('code', [...PILOT_CODES]),
        ...
        pilotStandards.sort((a, b) => PILOT_CODES.indexOf(a.code as typeof PILOT_CODES[number]) - PILOT_CODES.indexOf(b.code as typeof PILOT_CODES[number]));
        setStandards(pilotStandards);

        const students = (rawStudents ?? []) as unknown as StudentRow[];
        if (students.length === 0) { setStudentData([]); setLoading(false); return; }
        ...
        const [
          ...
          supabase
            ...
            .select('student_id, standard_id, current_status, sessions_passed, sessions_attempted, failed_turns, teacher_flag, teacher_flag_reason, mastered_at')
            .in('student_id', studentIds),
          supabase
            .from('sessions')
            .select('id, student_id, standard_id, phase, started_at, completed_at, dominant_classification, mastery_achieved, time_spent_seconds')
            .in('student_id', studentIds)
        ...
        const { data: schemaRespData } = schemaIds.length > 0
          ? await supabase
        ...
        for (const r of (schemaRespData ?? []) as { schema_intervention_id: string; readiness_score: number }[]) {
  ...
  const standardsMastered   = studentData.reduce((n, sd) =>
    n + sd.progress.filter((p) => p.current_status === 'mastered').length, 0);
  const inInterventionCount = studentData.filter((sd) =>
    sd.progress.some((p) => ['intervening', 'practicing'].includes(p.current_status))).length;
  const needsAttentionCount = studentData.reduce((n, sd) =>
    n + sd.progress.filter((p) => p.teacher_flag === true).length, 0);
  ...
  const COL_GRID = '180px 90px 1fr 1fr 1fr 80px';
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F9FA', fontFamily: FONTS.ui, display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '0 20px 40px', width: '100%', boxSizing: 'border-box' as const }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 0 12px' }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: C.gray, marginTop: 2 }}>
              {studentsActive} student{studentsActive !== 1 ? 's' : ''} · 3 standards · Q1 2026–27 pilot
            </div>
          </div>
          {currentTime && (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
          <StatCard value={studentsActive}      label="Students Active"    color={C.navy}  sub="Pilot cohort"           />
          <StatCard value={standardsMastered}   label="Standards Mastered" color={C.green} sub="Across all students"    />
          <StatCard value={inInterventionCount} label="In Intervention"    color={C.amber} sub="Active remediation"     />
          <StatCard value={needsAttentionCount} label="Needs Attention"    color={C.red}   sub="Teacher review required"/>
        </div>
        ...
        <div style={{ background: C.white, border: `0.5px solid ${C.border}`, borderRadius: 10, overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            background: '#F8F8F8', borderBottom: `1px solid ${C.border}`,
            padding: '8px 14px', gap: 8,
          }}>
            {['Student', 'Pre-Lit', 'ELA.9.R.1.1', 'ELA.9.R.1.2', 'ELA.9.R.2.1', 'Flags'].map((h) => (
              <div key={h} style={{ fontSize: 9, fontWeight: 700, color: C.gray, textTransform: 'uppercase' as const, letterSpacing: 1 }}>
                {h}
              </div>
            ))}
          </div>
          ...
          {studentData.length === 0 && (
            <div style={{ padding: '32px 14px', textAlign: 'center' as const, color: C.gray, fontSize: 13 }}>
              No students found for this cohort. Add students via Supabase with teacher_id = {user?.id}.
            </div>
          )}
            ...
            return (
              <div key={sd.student.id}>
                ...
                <div
                  ...
                  onMouseLeave={(e) => { if (!isExpanded) (e.currentTarget as HTMLDivElement).style.background = C.white; }}
                >
                  {/* Student */}
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: C.dark }}>{sd.student.full_name}</div>
                  ...
                  {standards.map((std) => {
                    ...
                    } else if (prog?.current_status === 'intervening' || prog?.current_status === 'practicing') {
                    ...
                    return (
                      <div key={std.id}>
                        ...
                        {subText && (
                          <div style={{ fontSize: 10, color: cfg.text, marginTop: 3, lineHeight: 1.3 }}>
                            {subText}
                          </div>
                        )}
                      </div>
