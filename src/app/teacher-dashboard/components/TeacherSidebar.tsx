...
export default function TeacherSidebar({
  ...
  return (
    <aside
      className={`fixed left-0 top-0 h-full bg-white border-r border-slate-200 flex flex-col z-40 transition-all duration-300 ${
        collapsed ? 'w-16' : 'w-60'
      }`}
    >
      {/* Logo */}
      <div className={`h-16 flex items-center border-b border-slate-100 flex-shrink-0 ${
        ...
        {!collapsed && (
          <span className="font-bold text-violet-700 text-xl tracking-tight">GOGI</span>
        )}
      </div>
              ...
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {collapsed && (
                      <div className="absolute left-full ml-2 px-2.5 py-1.5 bg-slate-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">
                        {item.label}
      ...
      <div className="border-t border-slate-100 p-2 flex flex-col gap-0.5">
        ...
        <button
          ...
          className={`sidebar-item sidebar-item-inactive text-rose-500 hover:bg-rose-50 hover:text-rose-600 ${collapsed ? 'justify-center px-2' : ''} group relative`}
        ...
        {/* Teacher profile */}
        {!collapsed && (
          <div className="flex items-center gap-2.5 px-3 py-2.5 mt-1 bg-violet-50 rounded-xl">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-slate-800 truncate">Mrs. Reyes</p>
              <p className="text-xs text-slate-500 truncate">8th Grade ELA</p>
            </div>
          </div>
        )}
      </div>
      ...
      <button
        ...
        className="absolute -right-3.5 top-20 w-7 h-7 bg-white border border-slate-200 rounded-full flex items-center justify-center shadow-sm hover:bg-violet-50 hover:border-violet-300 transition-all z-50"
      >
        {collapsed ? (
