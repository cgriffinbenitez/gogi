...
function ProgressContent() {
  ...
  const readingWinLogged = searchParams.get('win') === '1';

  // ── Labels per session count ──────────────────────────────────────────────

  const sessionLabel = readingWinLogged
    ? 'READING WIN LOGGED'
    : sessions === 2
      ? 'SESSION 2 COMPLETE'
      : 'SESSION 1 COMPLETE';

  const message = readingWinLogged
    ? 'That move is in your record.'
    : sessions === 2
      ? 'Two sessions in. One more to confirm it.'
      : "Good work. You're building this skill.";

  const subtext = readingWinLogged
    ? 'GOGI saved what you practiced today. Next time, you will prove it with a fresh question.'
    : sessions === 2
      ? "You're close. Come back tomorrow."
      : 'Come back tomorrow to keep going.';

  const subNote = readingWinLogged
    ? 'Daily win complete: learn the move, try it, log the evidence.'
    : sessions === 2
  ...
  return (
    ...
    >
      ...
      {/* Gogi */}
      <GogiAvatar size={80} state={readingWinLogged ? 'celebrate' : 'engaged'} />
